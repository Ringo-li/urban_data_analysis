...
ak=ZMtqRARPIk09nUpWnNKSmMPBVpwTvsci&page_size=20&page_num={}'.format(self.query,self.bounds,i)
            urls.append(url)
        return urls
...
# '政府机构':['中央机构','各级政府','行政单位','公检法机构','涉外机构','党派团体','福利机构','政治教育机构','社会团体','民主党派','居民委员会']
    ...
    for h1,v in pois.items():
        ...
        for loc_to_use in locs_to_use[:2]:
            coor_use = ','.join(loc_to_use)
            print('coor {} starting ...'.format(coor_use))
            for h2 in v:
